...
class bbb_noise_fbm_tilde : public c74::min::object<bbb_noise_fbm_tilde>, public c74::min::mc_operator<> {
public:
	MIN_DESCRIPTION{"Fractal Brownian Motion noise signal generator"};
	MIN_TAGS{"noise, fbm, fractal, procedural, audio"};
	MIN_AUTHOR{"2bit"};
...
private:
	bbb::noise::evaluator eval_{
		bbb::noise::config{bbb::noise::type::simplex, 0, bbb::noise::cellular_mode::f1,
		                   bbb::noise::fractal_type::fbm, bbb::noise::type::simplex, 4, 0.5, 2.0}
	};
};

MIN_EXTERNAL(bbb_noise_fbm_tilde);
